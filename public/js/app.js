...
const updateUI = async () => {
  ...
  // NEW - add logic to show/hide gated content after authentication
    ...
    let myData = await localStorage.getItem("testing");
    setTimeout(function() {
      let token = localStorage.getItem('testing');
        }, 50);


    document.getElementById("ipt-user-rules").innerHTML = 
    setTimeout;
    
    // JSON.stringify(
    //   await AsyncStorage.getItem('something')
    //     .then(value => this.setState({ something: value}))
...
// asyncStorage = new AsyncStorage('./asyncscratch');
...
//         // localStorage.setItem("webSocketTest", JSON.parse(message));
//         console.log("received on client side!");
//     });
// });
